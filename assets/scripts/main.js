...
function initFormHandler() {
  ...
  ref.addEventListener('submit', ()=> {
    let formData = new FormData(ref);
    let recipeObject = {};
    for (let data of formData.keys()){
      recipeObject.data = formData.get(data);
    }
    let recipeCard = document.createElement('recipe-card');
    recipeCard.data = recipeObject;
    document.querySelector('main').append(recipeCard);
    let newRecipe = localStorage.getItem('recipes').push(recipeCard);
  ...
  let clsRef = document.querySelector(".danger");
  // B11. TODO - Add a click event listener to clear local storage button
  clsRef.addEventListener('click', ()=> {
    localStorage.clear();
